...
export default class Game extends EventEmitter<EventMap> {
  ...
  get focusedHand(): Hand {
    return this.focusedPlayer.getHand(this.state.focusedHandIndex);
  }

  get focusedPlayer(): Player {
    const index = Math.min(
      this.state.focusedPlayerIndex,
      this.players.length - 1,
    );
    return this.players[Math.max(index, 0)];
  }
    ...
    try {
      switch (step) {
        ...
        case GameStep.WaitingForPlayInput:
          ...
          step = this.playPlayerTurns(input);
          break;

        case GameStep.PlayHandsLeft:
          // All players have taken their turns; only the dealer remains.
          this.playDealer();
          step = GameStep.WaitingForNewGameInput;
          break;
  ...
  playPlayerTurns(input?: Move): GameStep {
    while (this.state.focusedPlayerIndex < this.players.length) {
      const player = this.players[this.state.focusedPlayerIndex];

      if (!player.isUser) {
        this.playNPCHands(player);
        this.state.focusedPlayerIndex += 1;
        this.state.focusedHandIndex = 0;
        continue;
      }

      const hand = this.focusedHand;

      if (input === undefined) {
        // Resolve hands that are decided without input (blackjack, dealer
        // blackjack) and advance to the next player that needs input.
        if (this.setblackjackWinner(player, hand)) {
          this.advancePlayerFocus(player);
          continue;
        }
        return GameStep.WaitingForPlayInput;
      }

      const handFinished = this.stepHand(player, hand, hand.betAmount, input);

      if (handFinished) {
        if (this.state.focusedHandIndex < player.handsCount - 1) {
          this.state.focusedHandIndex += 1;
        } else {
          // This player has no hands left that need input.
          this.state.focusedPlayerIndex += 1;
          this.state.focusedHandIndex = 0;
          input = undefined;
          continue;
      ...
      while (
        this.state.focusedHandIndex < player.handsCount - 1 &&
        this.focusedHand.finished
      ) {
        this.state.focusedHandIndex += 1;
      }

      if (this.focusedHand.finished) {
        // All of this player's hands are finished.
        this.state.focusedPlayerIndex += 1;
        this.state.focusedHandIndex = 0;
        input = undefined;
        continue;
  ...
  private advancePlayerFocus(player: Player): void {
    if (this.state.focusedHandIndex < player.handsCount - 1) {
      this.state.focusedHandIndex += 1;
    } else {
      this.state.focusedPlayerIndex += 1;
  ...
  removeCards(): void {
    ...
    this.state.focusedHandIndex = 0;
    this.state.focusedPlayerIndex = 0;
